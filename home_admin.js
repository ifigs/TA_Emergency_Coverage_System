...
function invite_admin(){
		var email = prompt("Please enter the Administrator's email:", "Administrator email");
		...
		if (email == null || email == "") {
			...
			$.ajax({
				...
				{
					var text = "Administrator with email "+email+" has permission code "+data+". Please email this code to the Administrator with instructions to create an account.";
...
function invite_ta(){
		...
		if (email == null || email == "") {
			...
			$.ajax({
				...
				success: function(data)
				{
					var text = "TA with email "+email+" has permission code "+data+". Please email this code to the TA with instructions to create an account.";
					alert(text); //alert the returned code and the email
				}
			});
		}
...
function invite_professor(){
	var email = prompt("Please enter the Professor's email:", "TA email");
	var account_type = "prof";
	if (email == null || email == "") {
		//alert( "User cancelled the prompt.");
		return false;
	} else {
		//somehow generate a random code and then send it to the php code to give it to the database
		// Serialize the data in the form
		var serializedData = '&email='+email+'&account_type='+account_type;
		//console.log(serializedData);

		// Fire off the request to invite.php
		$.ajax({
			url: "invite.php",
			type: "POST",
			data: serializedData,
			success: function(data)
			{
				var text = "Professor with email "+email+" has permission code "+data+". Please email this code to the professor with instructions to create an account.";
				alert(text); //alert the returned code and the email
			}
		});
	}
